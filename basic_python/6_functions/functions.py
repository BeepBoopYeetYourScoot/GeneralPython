...
# Когда мы выполняет последовательность функций, каждая из которых вызывает другую,
# main -> А() -> В() -> С() -> D(), мы сталкиваемся с понятием СТЕК ВЫЗОВОВ (CALL STACK)
# |                       |
# |__D(),_строка_вызова___|
# |__C(),_строка_вызова___|
# |__В(),_строка_вызова___|
# |__А(),_строка_вызова___|
# |__main,_строка_вызова__|
# |______CALL__STACK______|

# У стека существует предельный уровень заполнения, значит, его можно переполнить
# При вызове функции D() может произойти ошибка. В таком случае Python вернёт не только
# функцию, с которой упала программа, и строку, куда она должна была вернуться: он также вернёт
# стек вызова функции.
# Питон выдаст контекст, в котором была вызвана функция (она, например, может вызываться
# не только опосредованно (ч-з ф-ю С()), но и ф-и А() и В() также могут непосредственно
# обращаться к функции D()
